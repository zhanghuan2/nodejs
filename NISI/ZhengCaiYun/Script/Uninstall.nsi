...
Function .onInit
  ...
  Exec '"$TEMP\$EXEFILE" /UnInstall $EXEDIR $R0'
  Abort
  ClearErrors
    ${GetParameters} $R0 # 获得命令行
  ...
  ${EndIf}
  ;初始化数据  安装目录
  nsSkinEngine::NSISInitSkinEngine /NOUNLOAD "$R0" "Uninstall_$(LANG_MESSAGE).xml" "WizardTab" "false" "${PRODUCT_NAME_EN}" "${PRODUCT_KEY}" "app.ico" "true"
   Pop $Dialog
   ;初始化MessageBox窗口
...
Section UninstallApp
    ...
    ${StdUtils.InvokeShellVerb} $0 "$varOldFileDir" "${MAIN_LAUNCHAPP_NAME}" ${StdUtils.Const.ShellVerb.UnpinFromTaskbar}
	${StdUtils.InvokeShellVerb} $0 "$varOldFileDir" "${MAIN_LAUNCHAPP_NAME}" ${StdUtils.Const.ShellVerb.UnpinFromStart}
